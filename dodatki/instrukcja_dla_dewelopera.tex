...
Katalog .gazebo/ standardowo jest tworzony podczas pierwszego uruchomienia programu Gazebo. W katalogu .gazebo/models/ przechowywane s± modele obiektów. W przypadku pracy z innym obrazem lub braku modeli nale¿y przed uruchomieniem symulatora utworzyæ katalog i dodaæ brakuj±ce modele.

Katalog catkin\_ws/ jest przeznaczony do pracy deweloperskiej. W podkatalogu catkin\_ws/src/nale¿y umie¶ciæ kod ¼ród³owy np. pluginu do programu Gazebo, programów pracuj±cych z systemem ROS lub inne.
...
Efektem kompilacji bêdzie utworzenie bibliotek pluginów z rozszerzeniem .so w katalogu /root/catkin\_ws/devel/lib/. Znajduj±ce siê w tej lokalizacji wtyczki s± wczytywane podczas uruchamiania Gazebo w zale¿no¶ci od ustawieñ zawartych w pliku .world, opisanych w rozdziale \ref{sec:pluginy}.
